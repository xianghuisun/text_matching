...
class Model:
    ...
    def add_abcnn2(self,p_tensor,h_tensor):
        ...
        p_mask=tf.cast(tf.sequence_mask(self.premise_length,maxlen=self.seq_length_p+self.filter_width-1),dtype=tf.float32)
        p_mask=tf.expand_dims(tf.expand_dims(p_mask,axis=1),axis=-1)#(batch_size,1,seq_length+2,1)
        h_mask=tf.cast(tf.sequence_mask(self.hypothesis_length,maxlen=self.seq_length_h+self.filter_width-1),dtype=tf.float32)
        h_mask=tf.expand_dims(tf.expand_dims(h_mask,axis=1),axis=-1)
        p_tensor=tf.multiply(p_tensor,p_mask)
        h_tensor=tf.multiply(h_tensor,h_mask)
    ...
    def add_abcnn1(self,p_tensor,h_tensor):
        ...
        seq_len=A.shape[-1]
        p_mask=tf.cast(tf.sequence_mask(self.premise_length,maxlen=self.seq_length_p),dtype=tf.float32)
        A=tf.multiply(A,tf.expand_dims(p_mask,axis=-1))
        h_mask=tf.cast(tf.sequence_mask(self.hypothesis_length,maxlen=self.seq_length_h),dtype=tf.float32)
        A=tf.multiply(A,tf.expand_dims(h_mask,axis=1))
        W=tf.Variable(tf.random_normal(shape=[int(seq_len),int(features)],dtype=tf.float32))
        ...
        return new_p,new_h#(batch_size,features,sequence_length,2)

    def wide_convolution1(self,input_tensor):
        #(batch_size,features,seq_length,1 or 21)
        '''
        ...
        return tf.transpose(out,perm=[0,3,2,1])#(batch_size,conv_filters,seq_length+2,1)
    def wide_convolution2(self,input_tensor):
        #(batch_size,features,seq_length,1 or 21)
        '''
        tf.nn.conv2d(input,filter,strides,padding)
        input a 4D tensor(NHWC),filter a 4D tensor
    ...
    def w_ap_layer(self,p_tensor,h_tensor):
        ...
        p_w_ap=tf.nn.avg_pool(p_tensor,ksize=[1,1,self.filter_width,1],strides=[1,1,1,1],padding="VALID")
        h_w_ap=tf.nn.avg_pool(h_tensor,ksize=[1,1,self.filter_width,1],strides=[1,1,1,1],padding="VALID")
    ...
    def final_pool_layer(self,x):
        #batch_size,conv_filters,seq_length,1)
        avg_pool=tf.reduce_mean(x,axis=2)
        max_pool=tf.reduce_max(x,axis=2)
        assert avg_pool.shape[-1]==max_pool.shape[-1]==1
        avg_pool=tf.reshape(avg_pool,shape=[-1,self.conv_filters])
        max_pool=tf.reshape(max_pool,shape=[-1,self.conv_filters])
    ...
    def forward(self):
        self.add_embedding()
        #(batch_size,embed_dim,sequence_length,1)
        p_tensor=self.p_embed
        h_tensor=self.h_embed
        p_mask=tf.cast(tf.sequence_mask(self.premise_length,maxlen=self.seq_length_p),dtype=tf.float32)
        h_mask=tf.cast(tf.sequence_mask(self.hypothesis_length,maxlen=self.seq_length_h),dtype=tf.float32)#(batch_size,seq_lengh)
        p_mask=tf.expand_dims(tf.expand_dims(p_mask,axis=1),axis=-1)
        h_mask=tf.expand_dims(tf.expand_dims(h_mask,axis=1),axis=-1)
        p_tensor=tf.multiply(p_tensor,p_mask)
        h_tensor=tf.multiply(h_tensor,h_mask)
        abcnn1_p,abcnn1_h=self.add_abcnn1(p_tensor,h_tensor)
        #(batch_size,embed_dim,sequence_length,2)
        assert abcnn1_p.shape[1]==p_tensor.shape[1] and abcnn1_h.shape[2]==h_tensor.shape[2]
        with tf.variable_scope("wide_conv",reuse=tf.AUTO_REUSE):
            wide_p=self.wide_convolution1(abcnn1_p)
        with tf.variable_scope("wide_conv",reuse=tf.AUTO_REUSE):
            wide_h=self.wide_convolution1(abcnn1_h)
        assert wide_p.shape[2]==abcnn1_p.shape[2]+self.filter_width-1 and wide_p.shape[-1]==1
        #(batch_size,conv_filters,seq_lenth+2,1)
        p_w_ap,h_w_ap=self.w_ap_layer(wide_p,wide_h)
        abcnn2_p,abcnn2_h=self.add_abcnn2(wide_p,wide_h)
        #(batch_size,conv_filters,sequence_length,1)
        self.assert_shape(p_w_ap,abcnn2_p)
        self.assert_shape(h_w_ap,abcnn2_h)

        p_tensor=tf.add(abcnn2_p,p_w_ap)
        h_tensor=tf.add(abcnn2_h,h_w_ap)
        ...
        #(batch_size,embed_dim,sequence_length,2)
        assert abcnn1_p.shape[1]==p_tensor.shape[1] and abcnn1_h.shape[2]==h_tensor.shape[2]
        with tf.variable_scope("wide_conv2",reuse=tf.AUTO_REUSE):
            wide_p=self.wide_convolution2(abcnn1_p)
        with tf.variable_scope("wide_conv2",reuse=tf.AUTO_REUSE):
            wide_h=self.wide_convolution2(abcnn1_h)
        assert wide_p.shape[2]==abcnn1_p.shape[2]+self.filter_width-1 and wide_p.shape[-1]==1
        #(batch_size,conv_filters,seq_lenth+2,1)
        abcnn2_p,abcnn2_h=self.add_abcnn2(wide_p,wide_h)
        #(batch_size,conv_filters,sequence_length,1)
        abcnn2_h=tf.multiply(abcnn2_h,h_mask)
        abcnn2_p=tf.multiply(abcnn2_p,p_mask)

        final_p=self.final_pool_layer(abcnn2_p)
        final_h=self.final_pool_layer(abcnn2_h)
        output=tf.concat([final_p,final_h],axis=-1)
        assert output.shape[-1]==4*self.conv_filters
        self.fc_out=tf.layers.dense(output,units=64,activation=tf.nn.tanh)
        self.train()