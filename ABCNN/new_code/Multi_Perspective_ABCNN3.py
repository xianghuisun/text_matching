...
class Model:
	def __init__(self,embedding_matrix,num_classes,seq_length_p,seq_length_h,filter_width=3):
		...
		self.l=20
		self.layers=2
		self.W1=tf.Variable(tf.random_normal(shape=[self.l,self.conv_filters],dtype=tf.float32))
		self.W2=tf.Variable(tf.random_normal(shape=[self.l,self.conv_filters],dtype=tf.float32))
		self.forward()
	...
	def add_abcnn2(self,p_tensor,h_tensor):
		...
		A=self.get_attention(p_tensor,h_tensor,self.W2)#(batch_size,l,p+2,h+2)
		p_attention=tf.reduce_sum(A,axis=3)#(batch_size,l,p+2)
